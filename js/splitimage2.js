...
var workers = {
    bpg: undefined, tng: undefined,
    jp2: undefined, jxr: undefined, webp: undefined
};
var nativeDec = {
    bpg: false, tng: false, jp2: false, jxr: false, webp: false,
...
function setImage(side, pathBase, codec, setText) {
    ...
    xhr.onload = function() {
        ...
        if (codec == 'bpg') {
            ...
            var tpg = new TuDecoder(canvas.getContext("2d"));
            tpg.onload = function() {
                canvas.width = tpg.imageData.width;
                canvas.height = tpg.imageData.height;
                canvas.getContext("2d").putImageData(tpg.imageData, 0, 0);
                processCanvasScale(canvas, side);
                window.URL.revokeObjectURL(blobPath);
            };
            tpg.load(blobPath);
...
function setSide(side) {
    ...
    if (pathBase != 'tucodec' && pathBase != 'Original' && urlFolder != 'psycomp/') {
